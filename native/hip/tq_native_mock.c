/**
 * tq_native_mock.c — TurboQuant HIP/ROCm mock C ABI implementation
 *
 * Implements every function declared in tq_native_api.h for the HIP backend
 * using purely CPU memory and the same deterministic LCG as the CUDA mock and
 * the Java CpuComputeSession.  No real HIP/ROCm headers are required.
 *
 * HIP-specific differences from native/cuda/tq_native_mock.c
 * ────────────────────────────────────────────────────────────
 * • tq_runtime_version() returns a ROCm-encoded integer:
 *     major * 10000 + minor * 100 + patch   (e.g. 60300 → ROCm 6.3.0)
 * • tq_device_name() returns "Mock HIP/ROCm Device N"
 * • tq_runtime_describe() returns "HIP mock 6.3 / device[N]: Mock HIP/ROCm Device N"
 *
 * Everything else — struct layouts, LCG constants, KV-cache simulation,
 * quantisation stubs — is identical to the CUDA mock so that benchmark
...
/* =========================================================================
 * ROCm version (mock: 6.3.0)
 * Encoding: major * 10000 + minor * 100 + patch
 * ========================================================================= */

#define MOCK_ROCM_VERSION    60300   /* ROCm 6.3.0 */

/* =========================================================================
 * Implementation structs (identical layout to CUDA mock)
 * ========================================================================= */
...
size_t tq_dtype_sizeof(tq_dtype_t dtype) {
    switch (dtype) {
        ...
        case TQ_DTYPE_UINT4:    return 1;
        default:                return 0;
    }
}

/* =========================================================================
 * Low-level device lifecycle
 * ========================================================================= */
...
tq_status_t tq_device_name(int32_t device_index, char* buf, size_t buf_len) {
    ...
    snprintf(buf, buf_len, "Mock HIP/ROCm Device %d", device_index);
    return TQ_OK;
}

int32_t tq_runtime_version(void) {
    return MOCK_ROCM_VERSION;
}

/* =========================================================================
 * Low-level stream management (no-ops on CPU mock)
 * ========================================================================= */
...
tq_status_t tq_stream_destroy(tq_stream_t stream)      { (void)stream; return TQ_OK; }
tq_status_t tq_stream_synchronize(tq_stream_t stream)  { (void)stream; return TQ_OK; }

/* =========================================================================
 * Low-level device memory (host malloc behind an opaque uint64_t)
 * ========================================================================= */
...
tq_status_t tq_upload_float32(
        tq_device_ptr_t dst, const float* src,
        size_t num_floats, tq_stream_t stream)
{
    (void)stream;
...
tq_status_t tq_download_float32(
        float* dst, tq_device_ptr_t src, size_t num_floats)
{
    if (!dst || !src) return TQ_ERR_INVALID_ARGUMENT;
...
/* =========================================================================
 * Low-level quantisation stubs
 * ========================================================================= */

tq_status_t tq_quantise_int8(
        tq_device_ptr_t dst, tq_device_ptr_t src,
        size_t numel, tq_device_ptr_t scale_out, tq_stream_t stream)
{
    ...
    if (scale_out) { float* sp = (float*)(uintptr_t)scale_out; *sp = scale; }
    return TQ_OK;
}
        ...
        size_t numel, float scale, tq_stream_t stream)
{
    ...
    for (size_t i = 0; i < numel; i++) fdst[i] = (float)isrc[i] * scale;
    return TQ_OK;
}

tq_status_t tq_quantise_uint4(
        tq_device_ptr_t dst, tq_device_ptr_t src,
        size_t numel, tq_stream_t stream)
{
    (void)dst; (void)src; (void)numel; (void)stream;
    return TQ_ERR_NOT_SUPPORTED;
}
...
tq_status_t tq_runtime_describe(tq_runtime_t rt, char* buf, size_t buf_len) {
    ...
    /* ROCm version encoding: major*10000 + minor*100 + patch */
    int ver   = tq_runtime_version();
    int major = ver / 10000;
    int minor = (ver % 10000) / 100;
    snprintf(buf, buf_len,
             "HIP mock %d.%d / device[%d]: Mock HIP/ROCm Device %d",
             major, minor, impl->device_index, impl->device_index);
    return TQ_OK;
}
 ...
 *
 * LCG constants are identical to CpuComputeSession and native/cuda/ so that
 * deterministic token output can be verified across all three paths.
 * ========================================================================= */
        ...
        tq_infer_result_t*  result_out)
{
    ...
    if (max_new_tokens > 0) logits[generated[max_new_tokens - 1]] = 1.0f;
